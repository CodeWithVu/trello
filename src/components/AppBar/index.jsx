...
function AppBar() {
  ...
  const barBgClass = isLight ? 'bg-white/60' : 'bg-black/20'

  return (
    <div className={`w-full gap-8 h-(--header-height) flex items-center justify-between ${board?.backgroundImage ? `${barBgClass} ${textColorClass} backdrop-blur-2xl` : 'bg-white text-black'} dark:bg-[#2c3e50]
       overflow-x-auto
    ...
    dark:[&::-webkit-scrollbar-thumb]:bg-neutral-400 `}>
      <div className="px-4 gap-4 flex items-center  dark:text-white ">
        ...
        <div className="hidden sm:flex gap-4 ">
          ...
          <Starred />
          <Template />
          <Button variant="primary" className={`flex items-center gap-1 ${textColorClass} dark:text-white  border-none hover:border-none hover:bg-transparent transition delay-150 duration-300 ease-in-out hover:-translate-y-1 hover:scale-110 `}>
      ...
      <div className={`px-4 gap-4 flex items-center ${board?.backgroundImage ? `${textColorClass}` : 'text-black'}  dark:text-white`}>
        ...
        <Tooltip content="Need help?" contentClassName="w-20">
          <IoMdHelpCircleOutline className="text-xl dark:text-amber-50" />
        </Tooltip>
        <Profile />
      </div>
    </div>
